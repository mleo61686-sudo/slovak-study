...
function main() {
  const mapPath = path.join(process.cwd(), "tmp", "sk-to-pl.json");
  if (!fs.existsSync(mapPath)) {
    console.error(`❌ Missing ${mapPath}. Run init-sk-to-pl.ts first.`);
    process.exit(1);
  }

  const SK_TO_PL = JSON.parse(
  ...
  for (const band of bands) {
    ...
    const plLessons = lessons.map((l) => {
      const title = toTitle((l as any).title);

      const words = (l.words ?? []).map((w) => {
        const skTerm = String(w.term ?? w.sk ?? "").trim();
        const plTermRaw = String(SK_TO_PL[skTerm] ?? "").trim();
        const plTerm = plTermRaw || `TODO:${skTerm}`;
    ...
    const constName = `PL_${band.toUpperCase()}_SOURCE`;
    const fileName = `pl-${band}.ts`;
    ...
    console.log(`✅ Wrote ${filePath} (${plLessons.length} lessons)`);
  }

  console.log(
    `\n👉 Next: connect pl-lessons-by-band.ts to these new sources (pl-a0/a1/a2/b1/b2).`
  );
}
